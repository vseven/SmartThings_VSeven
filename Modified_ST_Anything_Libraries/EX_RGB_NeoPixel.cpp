...
//private
	void EX_RGB_NeoPixel::writeCommandToOutput()
	{
		int subStringR;
		int subStringG;
		int subStringB;
		...
		uint32_t c = m_nStrip.Color(subStringR, subStringG, subStringB);
		
		if (st::Executor::debug) {
			Serial.println("Converted color value being sent: " + String(c));
		}

		m_nStrip.fill(c, 0, (n-1));
...
//public
	//constructor
	EX_RGB_NeoPixel::EX_RGB_NeoPixel(const __FlashStringHelper *name, byte pinRGB, byte count):
		Executor(name)
	{
	...
	//destructor
	EX_RGB_NeoPixel::~EX_RGB_NeoPixel()
	{
	
	}
	
	void EX_RGB_NeoPixel::init()
	{
	...
	void EX_RGB_NeoPixel::beSmart(const String &str)
	{
		String s=str.substring(str.indexOf(' ')+1);
		if (st::Executor::debug) {
			Serial.print(F("EX_RGB_NeoPixel::beSmart s = "));
			Serial.println(s);
		}
		if(s==F("on"))
		{
	...
	void EX_RGB_NeoPixel::refresh()
	{
		Everything::sendSmartString(getName() + " " + (m_bCurrentState == HIGH?F("on"):F("off")));
	}
	
	void EX_RGB_NeoPixel::setupPixels(byte pin, byte count)
	{
		m_nPinRGB=pin;
		m_nPixelCount=count;
	...
	Adafruit_NeoPixel EX_RGB_NeoPixel::m_nStrip;  // NeoPixel library object
