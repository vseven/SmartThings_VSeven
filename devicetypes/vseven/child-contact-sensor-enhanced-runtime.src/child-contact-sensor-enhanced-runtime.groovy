...
def updated() {
	...
	updateRuntime(device.currentValue("totalRuntimeInMilliseconds"))
}
...
def generateEvent(String name, String value) {
    ...
    // Update runtime
    if (device.latestValue("contact") == calculateRuntime) {
         log.debug("Status of contact matches when we should be tracking so add to runtime")
         ...
         updateRuntime(myTotal)
    }
    
	// Update device
	sendEvent(name: name,value: value)
    
	// Update lastUpdatedDate and the lastUpdated display string
    sendEvent(name: "lastUpdatedDate", value: currentDate.time, displayed: false)
	def nowDay = new Date().format("MMM dd", location.timeZone)
	def nowTime = new Date().format("h:mm a", location.timeZone)
...
def updateRuntime (Number value) {
	def myRuntimeDisplay
    if (runtimeUnits.equals("Hours")) {
    	if (value > 0) {
        	myRuntimeDisplay = Math.round((value / 36000000) * 100) / 100 + " Hours"
        } else {
        	myRuntimeDisplay = "0 Hours"
        }
	} else if (runtimeUnits.equals("Minutes")) {
       	if (value > 0) {
			myRuntimeDisplay = Math.round((value / 600000) * 10) / 10 + " Minutes"
        } else {
        	myRuntimeDisplay = "0 Minutes"
        }
	} else {
       	if (value > 0) {
			myRuntimeDisplay = Math.round(value / 10000) + " Seconds"
		} else {
