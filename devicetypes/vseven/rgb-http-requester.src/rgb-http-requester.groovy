 ...
 *    2017-03-16  Allan (vseven) Modified DTH for use with stand alone RGB webserver calls.  Renamed DTH for new purpose.
 * 
 */

// for the UI
metadata {
	definition (name: "RGB HTTP Requester", namespace: "vseven", author: "Allan (vseven)") {
	capability "Switch"		
  ...
  preferences {
		section("prefs") {
			input(name: "webServerIP", type: "text", title: "Enter the IP address of the RGB webserver.", required: true)
		}
	}
...
def off() {
    ...
    // How do we handle a off?  For now sending a #000000
    adjustColor("#000000")
}
...
def adjustColor(colorInHEX) {
    ...
    // Send to the device
    sendToWebServer("all?rgb=" + adjustedColor)
}

def sendToWebServer(String stringToSend) {
